 ...
 */
public class PlayerFoundMessage implements Runnable {
    public PlayerFoundMessage(Apollo plugin, String serverName, String playerName) {
    ...
    public void run() {
        synchronized (plugin.getConnect()) {
            ...
            try {
                MessageRequest messageRequest = new MessageRequest(serverName, "Apollo.playersearch.Found", playerName);
                ...
                if (mr.getStatusCode() != StatusCode.SUCCESS) {
                    System.out.println("[Apollo] Failed to send found messages");
                }
